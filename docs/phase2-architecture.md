...
| 2f | PVH/VBE framebuffer fallback | 🟡 PARTIAL — Bochs VBE probe (`kernel/vbe.c`) wired PVH-only into `fb_init()`; the compile-time guard bug (draw target set only under `#ifndef JOE_PVH_BOOT`) is FIXED — `fb_init()` now sets the draw target whenever `fb_addr != 0` on both paths — and `kernel.curlee` renders a single PVH frame + `FB: 1` marker. `make qemu-pvh-fb-smoke` gate added. **Acceptance NOT green**: on this host, QEMU's `-kernel` PVH loader maps the kernel image's `.bss`/`.data` read-only, so the probe's framebuffer-state stores are silently discarded and `fb_ready()` stays 0 (evidence-backed: minimal-kernel tests + kernel serial trace; the previous agent's "needs upstream QEMU fix" conclusion was wrong — a writable high-RAM state block is the in-repo fix, see `docs/phase2f-report.md`). GRUB path fully green. |
