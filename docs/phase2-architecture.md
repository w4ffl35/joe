...
| 2g | virtio-blk storage driver (raw sector read) | ✅ DONE — gh issue #26: `kernel/virtio_blk.curlee` (legacy virtio 0.9.5 over the PCI I/O BAR, 3-descriptor NEXT chain read requests, one request queue — the virtio-net ring pattern), `scripts/make-blk-disk.sh` (build-side raw image + model blob at a known LBA, no filesystem), `make qemu-blk-smoke` (boots the GRUB ISO with `-device virtio-blk-pci,disable-modern=on`, asserts `BLK: 1 -> BLK: 2 -> BLK: 3` — the read of the build-time "JOE-BLK!" pattern at LBA 64 is verified byte-for-byte). Direct prerequisite for Native Inference model loading beyond the compile-time-embedded budget. See `plans/phase2g-storage.md` |
